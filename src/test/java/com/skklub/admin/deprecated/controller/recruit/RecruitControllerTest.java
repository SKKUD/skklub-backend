package com.skklub.admin.deprecated.controller.recruit;
        ...
        actions.andExpect(status().isOk())
                ...
                .andDo(
                        document("recruit/create",
                                ...
                                queryParameters(
                                        parameterWithName("recruitStartAt").description("모집 시작일").attributes(RestDocsUtils.example("yyyy-MM-ddTHH:mm(T는 날짜랑 시간 구분용 문자)")).optional(),
                                        parameterWithName("recruitEndAt").description("모집 종료일").attributes(RestDocsUtils.example("2012-06-02T14:04(상시모집은 시작일 종료일 null or No Field)")).optional(),
                                        parameterWithName("recruitQuota").description("모집 정원 - String value").attributes(RestDocsUtils.example("00명 || 최대한 많이 뽑을 예정")),
                                        parameterWithName("recruitProcessDescription").description("모집 방식").attributes(RestDocsUtils.example("1. 어쩌구 2. 어쩌구 AnyString")),
                                        parameterWithName("recruitContact").description("모집 문의처").attributes(RestDocsUtils.example("010 - 1234 - 1234 || 인스타 아이디")).optional(),
                                        parameterWithName("recruitWebLink").description("모집 링크").attributes(RestDocsUtils.example("www.xxx.com || or any String")).optional()
                                ),
                                responseFields(
                                        fieldWithPath("id").type(WireFormat.FieldType.STRING).description("동아리 ID").attributes(RestDocsUtils.example("0")),
                                        fieldWithPath("name").type(WireFormat.FieldType.STRING).description("동아리 이름").attributes(RestDocsUtils.example("클럽 SKKULOL"))
                                )
                        )
                );
        ...
        actions.andExpect(status().isOk())
                ...
                .andDo(
                        document("recruit/update",
                                pathParameters(
                                        parameterWithName("clubId").description("모집중인 동아리 ID").attributes(RestDocsUtils.example("1"))
                                ),
                                queryParameters(
                                        parameterWithName("recruitStartAt").description("모집 시작일").attributes(RestDocsUtils.example("yyyy-MM-ddTHH:mm(T는 날짜랑 시간 구분용 문자)")).optional(),
                                        parameterWithName("recruitEndAt").description("모집 종료일").attributes(RestDocsUtils.example("2012-06-02T14:04(상시모집은 시작일 종료일 null or No Field)")).optional(),
